    ...
    <div id="container">
      ...
      <div id="dashboard">
        ...
        <section id="solar-card" className="card">
          <h3>
            <i className="bi bi-sun" /> Datos solares
          </h3>
          {snapshot.solarCycle ? (
            <>
              <p>
                <i className="bi bi-exclamation-triangle" /> {snapshot.solarCycle.statusLine}
              </p>
              <p>
                <i className="bi bi-clock" /> {snapshot.solarCycle.durationLine}
              </p>
              <p>
                <i className="bi bi-brightness-high" /> Mediodía: {snapshot.solar.solarNoon.local}{' '}
                <span className="small">({snapshot.solar.solarNoon.santiago})</span>
              </p>
              <p>
                <i className="bi bi-compass" /> Elevación máxima: {snapshot.solar.maxElevationDeg}°
              </p>
              <p>
                <i className="bi bi-clock" /> Día: {snapshot.solar.dayLength} &nbsp;|&nbsp; Noche:{' '}
            ...
            <>
              <p>
                <i className="bi bi-sunrise" /> Amanecer: {snapshot.solar.sunrise.local}{' '}
                <span className="small">({snapshot.solar.sunrise.santiago})</span>
              </p>
              <p>
                <i className="bi bi-sunset" /> Atardecer: {snapshot.solar.sunset.local}{' '}
        ...
        <section id="lunar-card" className="card">
          <h3>
            <i className="bi bi-moon-stars" /> Datos lunares
          </h3>
          <p>
            Fase: {snapshot.lunar.phaseName} — {snapshot.lunar.illumination}%
          </p>
          <p>
            <i className="bi bi-arrow-up-circle" /> Salida: {snapshot.lunar.moonrise.local}{' '}
            <span className="small">({snapshot.lunar.moonrise.santiago})</span>
          </p>
          <p>
            <i className="bi bi-arrow-down-circle" /> Puesta: {snapshot.lunar.moonset.local}{' '}
        ...
        <section id="solar-arc-card" className="card">
          <h3>
            <i className="bi bi-sun" /> Trayectoria solar
          </h3>
        ...
        <section id="special-light-card" className="card">
          <h3>
            <i className="bi bi-brightness-alt-high" /> Luz especial
          </h3>
          <p>
            Golden AM: {snapshot.specialLight.goldenAM.start.local} → {snapshot.specialLight.goldenAM.end.local}{' '}
            <span className="small">({snapshot.specialLight.goldenAM.start.santiago} → {snapshot.specialLight.goldenAM.end.santiago})</span>
          </p>
          <p>
            Golden PM: {snapshot.specialLight.goldenPM.start.local} → {snapshot.specialLight.goldenPM.end.local}{' '}
            <span className="small">({snapshot.specialLight.goldenPM.start.santiago} → {snapshot.specialLight.goldenPM.end.santiago})</span>
          </p>
          <p>
            Blue AM: {snapshot.specialLight.blueAM.start.local} → {snapshot.specialLight.blueAM.end.local}{' '}
            <span className="small">({snapshot.specialLight.blueAM.start.santiago} → {snapshot.specialLight.blueAM.end.santiago})</span>
          </p>
          <p>
            Blue PM: {snapshot.specialLight.bluePM.start.local} → {snapshot.specialLight.bluePM.end.local}{' '}
            <span className="small">({snapshot.specialLight.bluePM.start.santiago} → {snapshot.specialLight.bluePM.end.santiago})</span>
          </p>
        </section>
        ...
        <section id="calendar-info-card" className="card">
          <h3>
            <i className="bi bi-info-circle" /> Información del día
          </h3>
        ...
        <section id="experimental-clock-card" className="card">
          <h3>
            <i className="bi bi-bezier2" /> Experimental
          </h3>
